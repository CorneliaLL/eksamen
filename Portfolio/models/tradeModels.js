...
async function createTrade({portfolioID, accountID, stockID, tradeType, quantity, price, fee, totalCost, date}) {
...
async function checkFunds(accountID, totalCost) {
    const pool = await connectToDB();
...
async function checkHoldings(portfolioID, stockID, quantity) {
    const pool = await connectToDB();
